...
class Master extends Actor with ActorLogging {
    ...
    var workers: Map[Address, ActorRef] = Map[Address, ActorRef]() // every address have single address
    var pendingRemoval: Map[Address, ActorRef] = Map[Address, ActorRef]()
    // If we have UnreachableMember, we need to quarantine worker from that node & if node is UP, we remove it from MAP.
    ...
    /**/
    override def receive: Receive = { // chaining PartialFunctions
        handleClusterEvent
            .orElse(handleWorkerRegistration)
            .orElse(handleJob)
    }

    def handleClusterEvent: Receive = {
        case MemberUp(member) if member.hasRole("worker")=>
            ...
            if(pendingRemoval.contains(member.address)) {
                ...
                // handle future of tuple handleWorkerRegistration
            }
    ...
    def createNode(port: Int, role: String, props: Props, actorName: String): ActorRef = {
        val config = ConfigFactory.parseString(
            s"""
               ...
               |""".stripMargin).withFallback(ConfigFactory.load("part2_clustering/clusteringExample.conf"))

...
object CentralElectionSystem extends App {
    def startNode(port: Int) = {
       ...
       """.stripMargin)
            .withFallback(ConfigFactory.load("part3_advanced_clustering/votingSystemSingleton.conf"))
...
/** Voting aggregator is a stateful actor.
 * It's receive handler contains Set of Strings and a Map of pools.
 * If the node that contains this singleton actor dies and this Voting aggregator gets moved
 *  to different node, Then the state would be lost and recreated in new node
 *  The state will not move alongside the actor.
 *
 * In order to keep the state, We will need to make Voting aggregator Actor a "PERSISTENT Actor"
 * */
